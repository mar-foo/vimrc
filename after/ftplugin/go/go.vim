...
"    if point != -1
"        let pkgname = a:name[:point-1]
"        let pkgdir = "/usr/lib/go/src/" . pkgname . "/"
"        for pkgfile in split(globpath(pkgdir, "*.go"))
"            let regexp = "^func a:name[point+1:]"
"            let declaration = filter(readfile(expand(pkgfile)), 'match(v:val, regexp) >= 1')
"            if declaration != []
...
"endfunc
"
"function Gocomplete(findstart, base)
"    if a:findstart
